...
caja addEventHandler ["ContainerOpened",
	...
	hint format ["Arsenal Unlocking Requirements\n\n\nWeapons: %1\nBackpacks: %5\nMagazines/Usables: %2\nOptics: %3\nVests: %3\nOther Items: %4",(_weapBase + (count unlockedWeapons) - (2*_fabricas)),(13 + (count unlockedMagazines)) - (2*_fabricas),(_itemBase - 10 + (count unlockedItems) - _fabricas),(_itemBase + (count unlockedItems) - _fabricas),5*(count unlockedBackpacks)];
	}
    ];
