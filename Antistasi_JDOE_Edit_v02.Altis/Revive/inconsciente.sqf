...
if (_unit getVariable "inconsciente") exitWith {diag_log "inconsciente 3"};
if (damage _unit < 0.9) exitWith {diag_log "inconsciente 4"};
if (!local _unit) exitWith {diag_log "inconsciente 5"};
...
if (time > _bleedOut) exitWith
	...
	if (_isPlayer) then
		{
		//_unit playMoveNow "AmovPpneMstpSnonWnonDnon_healed"; // NOTE: JDOE Edit
		_ayudado = _unit getVariable "ayudado";
		if (!isNil "_ayudado") then
			{
			_ayuda = [_unit] call pedirAyuda;
...
if (alive _unit) then
	{
	//_unit playMoveNow "AmovPpneMstpSnonWnonDnon_healed"; // MOTE: JDOE Edit
	if (!_isPlayer) then
		{
