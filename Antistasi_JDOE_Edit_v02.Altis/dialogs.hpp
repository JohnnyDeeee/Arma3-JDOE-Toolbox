...
#define STR_HQ_FIA "if (hayBE) then {[] remoteExec [""fnc_BE_buyUpgrade"", 2]} else {closeDialog 0; nul = [] call FIAskillAdd;}"
...
BTN_L3(-1, "Move HQ to another Zone", "", "closeDialog 0; nul = [] spawn moveHQ;");

BTN_R1(-1, "Ingame Member's List", "", "if (isMultiplayer) then {nul = [] execVM ""OrgPlayers\membersList.sqf""} else {hint ""This function is MP only""};");
BTN_R2(109, "Train FIA", "", STR_HQ_FIA);
BTN_R3(-1, "Rebuild Assets", "Cost: 5.000 €", "closeDialog 0; nul = [] execVM ""rebuildAssets.sqf"";");

BTN_M(BTN_Y_4, -1, "Garage Access", "", "closeDialog 0; nul = [false] spawn garage;");
	};
};
...
#define STR_LOAD_YES "closeDialog 0; if ((player == Stavros) and (isNil ""placementDone"")) then {[""statSave\loadAccount.sqf"",""BIS_fnc_execVM""] call BIS_fnc_MP; placementDone = true; publicVariable ""placementDone""} else {nul = [] execVM ""statSave\loadAccount.sqf"";};"
...
BTN_M(BTN_Y_2, -1, "Done", "", "if (!isNil ""switchCom"") then {closedialog 0; nul = [] execVM ""Dialogs\membersMenu.sqf"";} else {hint ""Select an option first""};");
	};
};
...
BTN_R2(-1,"Manage Camps", "Establish/Abandon Camps", "closeDialog 0; nul= createDialog ""camp_dialog"";");

BTN_M(BTN_Y_3, -1, "HQ Fortifications", "", "closeDialog 0; nul= createDialog ""HQ_fort_dialog"";");
	};
};
...
BTN_M(BTN_Y_5, -1, "NATO QRF", "Cost: 10 points", "closeDialog 0; [""NATOQRF""] execVM ""NatoDialog.sqf"";");
	};
};
...
BTN_BACK("closeDialog 0; vehQuery = nil; nul= [] execVM ""Dialogs\squad_recruit.sqf"";");
...
BTN_L1(-1, "Add Server Member", "", "if (isMultiplayer) then {closeDialog 0; nul = [""add""] call memberAdd;} else {hint ""This function is MP only""};");
BTN_L2(-1, "Remove Server Member", "", "if (isMultiplayer) then {closeDialog 0; nul = [""remove""] call memberAdd;} else {hint ""This function is MP only""};");
...
BTN_M(BTN_Y_2, -1, "Done", "", "if (!isNil ""miembros"") then {closedialog 0; nul = [] execVM ""Dialogs\firstLoad.sqf"";} else {hint ""Select an option first""};");
...
BTN_L1(-1, "Garage\Sell Vehicle", "", "closeDialog 0; nul = createDialog ""garage_sell"";");
BTN_R1(-1, "Vehicles and Squads", "", "closeDialog 0; if (player == Stavros) then {nul = createDialog ""squad_manager""} else {hint ""Only Player Commander has access to this function""};");

BTN_M(BTN_Y_2, -1, "Unlock Vehicle", "", "closeDialog 0; if !(isMultiplayer) then {hint ""It's unlocked already.""} else {if (player != stavros) then {nul = [false] call unlockVehicle} else {nul = [true] call unlockVehicle};};");
...
BTN_L1(-1, "Garage Vehicle", "", "closeDialog 0; if (player != stavros) then {nul = [false] call garageVehicle} else {if (isMultiplayer) then {createDialog ""garage_check""} else {nul = [true] call garageVehicle}};");
BTN_R1(-1, "Sell Vehicle", "", "closeDialog 0; if (player == stavros) then {nul = [] call sellVehicle} else {hint ""Only the Commander can sell vehicles""};");
...
BTN_L1(-1, "Personal Garage", "", "closeDialog 0; nul = [false] call garageVehicle;");
BTN_R1(-1, "FIA Garage", "", "closeDialog 0; nul = [true] call garageVehicle;");
...
BTN_BACK("closeDialog 0; nul = createDialog ""NATO_Options"";");
...
BTN_L1(-1, "HE", "", "closeDialog 0; if (hayUSAF) then {tipoMuni = ""RHS_mag_m1_he_12"";} else {tipoMuni = ""32Rnd_155mm_Mo_shells"";}");
BTN_R1(-1, "Laser Guided", "", "closeDialog 0; tipoMuni = ""2Rnd_155mm_Mo_LG"";");

BTN_M(BTN_Y_2, -1, "Smoke", "", "closeDialog 0; if (hayUSAF) then {tipoMuni = ""rhs_mag_m60a2_smoke_4"";} else {tipoMuni = ""6Rnd_155mm_Mo_smoke"";}");
...
class fasttravel_dialog // 340
{
	...
	BTN_L1(-1, "Fast Travel (old)", "Targets: all FIA-controlled zones", "closeDialog 0; nul = [] execVM ""fastTravelRadio.sqf"";");
	BTN_R1(-1, "Fast Travel (new)", "Only FIA camps and HQ", "closeDialog 0; nul = [] execVM ""altFastTravel.sqf"";");
...
class com_menu // 360
{
	...
	#define STR_COM_FIA "closeDialog 0; if (player == Stavros) then {[""status""] remoteExecCall [""fnc_infoScreen"", 2]};"
	...
	BTN_R1(-1, "Additional Options", "Toggle AXP system, easy mode, old fast-travel system, etc", "closeDialog 0; nul = createDialog ""com_options"";");
	BTN_R2(-1, "Maintenance", "Resync the arsenal, run gear checks, update FIA progress, etc", "closeDialog 0; nul = createDialog ""maintenance_menu"";");
...
class boost_menu // 390
{
	...
	#define STR_BST_NO "closeDialog 0; [false] remoteExec [""fnc_MAINT_arsenal"", 2]; if (hayBE) then {[] call fnc_BE_refresh}; if ((player == stavros) and (isNil ""placementDone"")) then {[] spawn placementselection};"
...
#define STR_GO_RSA "closeDialog 0; if (player == stavros) then {[] remoteExec [""fnc_MAINT_main"", 2];};"

BTN_L1(-1, "Civ Config", "", "closeDialog 0; nul = createDialog ""civ_config"";");
BTN_L2(-1, "Commander Menu", "", "closeDialog 0; nul = createDialog ""com_menu"";");
BTN_L3(-1, "Music ON/OFF", "", "closedialog 0; if (musicON) then {musicON = false; hint ""Music turned OFF"";} else {musicON = true; nul = execVM ""musica.sqf""; hint ""Music turned ON""};");
BTN_L4(-1, "FPS Limiter", "", "closeDialog 0; nul = createDialog ""fps_limiter"";");
...
BTN_L1(-1, "Fast Travel", "", "closeDialog 0; if (server getVariable ""enableFTold"") then {nul= createDialog ""fasttravel_dialog""} else {nul = [] execVM ""altFastTravel.sqf""};");
BTN_L2(-1, "Disguise Yourself", "", "closeDialog 0; nul = [] spawn undercover");
BTN_L3(-1, "Vehicle Manager", "", "closeDialog 0; nul = createDialog ""vehicle_manager"";");
BTN_L4(-1, "AI Management", "", "if (player == leader group player) then {closeDialog 0; nul = createDialog ""AI_management""} else {hint ""Only group leaders may access to this option""};");

BTN_R1(-1, "NATO Options", "", "closeDialog 0; nul=CreateDialog ""NATO_Options"";");
BTN_R2(-1, "Recruit Squad", "", "closeDialog 0; nul= [] execVM ""Dialogs\squad_recruit.sqf"";");
BTN_R3(-1, "Building Options", "", "closeDialog 0; nul=CreateDialog ""build_menu"";");
BTN_R4(-1, "Player and Money", "", "closeDialog 0; if (isMultiPlayer) then {nul = createDialog ""player_money""} else {hint ""MP Only Menu""};");

BTN_L5(-1, "Resign Commander", "", "closedialog 0; if (isMultiplayer) then {execVM ""orgPlayers\commResign.sqf""} else {hint ""This feature is MP Only""};");
BTN_R5(-1, "Extra options", "JohnDoe's extra options", "closedialog 0; nul=CreateDialog ""JDOE_options"";"); // NOTE: JDOE Edit
	};
};
...
class JDOE_options
{
	...
	{
		AS_BOX_D(BOX_H_10);
		AS_FRAME_D(FRAME_H_10, "Extra Options");
		BTN_BACK("closedialog 0; if (stavros==player) then {nul = createDialog ""radio_comm_commander""} else {nul = createDialog ""radio_comm_player""};");

		#define STR_GO_PSS "closeDialog 0; [""statSave\saveLoop.sqf"",""BIS_fnc_execVM""] call BIS_fnc_MP;"

		BTN_L1(-1, "Enable fatique", "", "nul = [true] execVM ""JDOE\Fatigue.sqf""; systemChat ""Your fatigue has been enabled."";");
		BTN_R1(-1, "Disable fatique", "", "nul = [false] execVM ""JDOE\Fatigue.sqf""; systemChat ""Your fatigue has been disabled."";");
		BTN_L2(-1, "Show FastTravel radius", "Shows the FastTravel radius as a blue marker on the map", "nul = [""create""] execVM ""JDOE\FastTravelRadiusMarkers.sqf""; systemChat ""FastTravel radius markers have been created on your map."";");
		BTN_R2(-1, "Hide FastTravel radius", "", "nul = [""delete""] execVM ""JDOE\FastTravelRadiusMarkers.sqf""; systemChat ""FastTravel radius markers have been removed from your map."";");

		BTN_M(BTN_Y_5, -1, "Persistent Save", "", STR_GO_PSS);
	};
};
...
BTN_R1(-1, "AI Management", "", "if (player == leader group player) then {closeDialog 0; nul = createDialog ""AI_management""} else {hint ""Only group leaders may access to this option""};");
BTN_R2(-1, "Player and Money", "", "closeDialog 0; if (isMultiPlayer) then {nul = createDialog ""player_money""} else {hint ""MP Only Menu""};");
BTN_R3(-1, "Vehicle Manager", "", "closeDialog 0; nul = createDialog ""vehicle_manager"";");

BTN_M(BTN_Y_5, -1, "Extra options", "JohnDoe's extra options", "closedialog 0; nul=CreateDialog ""JDOE_options"";"); // NOTE: JDOE Edit
	};
};
...
BTN_L1(-1, "Yes", "", "closeDialog 0; [] spawn buildHQ");
BTN_R1(-1, "No", "", A_CLOSE);
...
class com_options
{
	...
	#define STR_COM_OPT_WPP "if (server getVariable [""enableWpnProf"",false]) then {server setVariable [""enableWpnProf"",false,true]; [] remoteExec [""fnc_resetSkills"", [0,-2] select isDedicated,true]} else {server setVariable [""enableWpnProf"",true,true]}; hint format [""Current setting: %1"", [""on"", ""off""] select (server getVariable [""enableWpnProf"",false])];"
...
class tfar_menu
{
	...
	BTN_L1(-1, "Save Radio Settings", "Save TFAR radio settings.", "closeDialog 0; [player] spawn fnc_saveTFARsettings");

	BTN_R1(-1, "Load Radio Settings", "Load previously saved TFAR radio settings.", "closeDialog 0; [player] spawn fnc_loadTFARsettings");
	};
};
